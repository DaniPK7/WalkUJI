namespace Mapbox.Unity.MeshGeneration.Factories
{
    ...
    public class CustomDirections : MonoBehaviour
	{
		[SerializeField]
		AbstractMap _map;
		...
		GameObject CreateGameObject(MeshData data)
		{
			...
			_directionsGO = new GameObject("waypoint");
		...
		{
			_waypoints[1] = _waypoints[0];

		}
		public void SetWaypoint()
		{
			if (DDScript.dropSelected == 0) 
			{ 
				print("No podes");
				return;
			}
			string coords = DDScript.GetDropdownSelection();
			print("cd coords: " + coords);


			_waypoints[1] = transform.GetChild(DDScript.dropSelected - 1).transform;

			//SpawnScript.AddNewLocation(coords);
		}
	}
}
